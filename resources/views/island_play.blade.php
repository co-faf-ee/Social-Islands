...
<div class="container is-fluid">
  <div class="notification" style='padding-left:10px;'>
    <center>
      ...
      var firebaseConfig = {
        apiKey: "AIzaSyAjx6c_TzaOLC2A0Ln33t7lNXm3Kdm5fHc",
        authDomain: "social-islands.firebaseapp.com",
        databaseURL: "https://social-islands.firebaseio.com",
        projectId: "social-islands",
        storageBucket: "",
        messagingSenderId: "910064435365",
        appId: "1:910064435365:web:bb468f61f6e1bcad"
      };
      firebase.initializeApp(firebaseConfig);
      var database = firebase.database(); // RT Database
      const IslandID = "IslandId" + {{$island->id}};
      var PlayersRef = firebase.database().ref(IslandID + "/Players/");
      var AuthUsername = "{{Auth::user()->username}}";
      var AuthAvatar = "{{Auth::user()->avatar_url}}";
      // End of Engine Init
      ...
      class Player{
        constructor(username, url) {
          ...
          this.UpdateX = function(speed,direction){
            ...
            if(collide == false){
              ...
              if(direction == "right"){
                ...
                var j;for(j = 0; j < Workspace.Players.length; j++){
                  if(Workspace.Players[j].username != AuthUsername){
                    Workspace.Players[j].x -= speed;
                  }
                }
          ...
          Workspace.Players.push(this); console.log("pushed");
        }
      }
      class Asset{
      ...
      // Prebuild
      var localPlayer = new Player(AuthUsername,AuthAvatar);
      ...
      var localPlayerTagName = new TextUI("12px Arial",localPlayer.x+3,localPlayer.y+16.5,AuthUsername);
      ...
      // Server serve functions
      firebase.database().ref(IslandID + "/Players/" + localPlayer.username).onDisconnect(serverRemovePlayer(localPlayer.username));//

      function uploadPlayer(player) {
        firebase.database().ref(IslandID + "/Players/" + player.username).set({
      ...
      /*function removePlayer(username){
        //Workspace.Players.remove(function(el){return el.username === username;});
        Workspace.Players = Workspace.Players.filter(function(player){return player.username !== username;});
        console.log(username + " disconnected!");
      }*/

      function serverRemovePlayer(username){
        firebase.database().ref(IslandID + "/Players/" + username).remove();
        //Workspace.Players = Workspace.Players.filter(function(player){return player.username !== username;});
        console.log(username + " disconnected!");
      }
      ...
      // Server serve (needed?)
      uploadPlayer(localPlayer);

      /*PlayersRef.on('child_removed', function(player){
        removePlayer(player.val().username);
      });*/

      PlayersRef.on('child_added', function(player){
        if(player.val().username != localPlayer.username){
          var globalPlayer = new Player(player.val().username,player.val().avatar);
        }
        console.log(player.val().username + " has connected!");
      });
